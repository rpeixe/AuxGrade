...
def get_missing_courses(user):
    ...
    missing_courses = []
    core_courses = DegreeCourse.objects.filter(course_type = "CO", degree__pk = user.degree.pk)
    for degreecourse in core_courses:
        if degreecourse.course not in user.finished_courses.all():
            missing_courses.append(degreecourse.course)
